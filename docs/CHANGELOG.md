...
## 2026-05-26 — 双均线日线 1 年回测专用脚本

- `scripts/backtest_dual_ma_1y.py`：只启用 ma_cluster_breakout + ma20_pullback，在日线级别跑过去 1 年（2025-05-26 → 2026-05-26）；输出累计/年化/胜率/最大回撤/赔率，按规则细分
- `scripts/backtest_dual_ma_compare.py`：参数对比矩阵 — 默认 vs cluster_w=1.5/3.0、exit=10/20 天，扫 BTC/ETH 各一遍出对比表
- 实测结论：日线下 ma_cluster_breakout 几乎不触发（6 均线难密集到 0.6%）；持仓 10→20 天 ETH 胜率从 42.9% 跳到 64.3%、累计 +63%→+148%、回撤从 -30% 降到 -9.5%；BTC 同期较弱（默认 -4.4%，exit=20 转 +3.3%）
